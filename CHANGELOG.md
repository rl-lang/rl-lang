...
- **Vector drawing (`std::gui`)** - `gui_canvas` regions with per-frame `gui_draw_line`/`rect`/`circle`/`text` (local coordinates, 0-255 colors). Command lists drain every render, so animated scenes redraw from `on_frame`; static scenes redraw identically. Cheaper than pixel-array sprites for shapes, HUDs, and diagrams.
