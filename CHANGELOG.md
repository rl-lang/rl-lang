...
- **`core::` intrinsics** - compiler-blessed primitives for self-hosting (VM only for now): `__arr_new/push/get/set`, `__map_new/get/set/keys`, `__set_new/add/has`, `__abort`, `__type_of`. Abort-on-misuse semantics (missing keys, out-of-bounds, non-sets), mirroring `Index`. The name `core` is reserved; numeric casts stay with the `as` operator.
