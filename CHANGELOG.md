...
- **Byte buffers (`core::__buf_*`)** - amortized-O(1) incremental byte accumulation behind a `handle(Buffer)` (new `HandleKind::Buffer = 6`): `new`/`len`/`push_byte`/`get_byte`/`set_byte`/`append`/`slice`/`clear`/`to_string`/`free` plus `__syscall6` bridge fns `addr`/`resize`. Misuse aborts (bad handle, out-of-bounds, split codepoint, negative resize); `to_string` is lossy and never aborts. VM and C backends behave identically (fixed 256-slot C table, growth doubles from 16, shrink/clear keep capacity). The primitive floor for an RL-written renderer, lexer, and C-emitter.
- **Worker threads (`core::__spawn`/`__emit`/`__poll`, `std::gui::gui_on_frame`)** - task parallelism without shared memory: `__spawn(source)` runs RL on a fresh `Vm` on a shared pool thread (CPU count workers, bounded queue of 256, saturation errs instead of blocking), `__emit(text)` streams progress from inside, `__poll(job)` reads non-blocking (`ok` progress/final, `err("pending")` while running, outcome reaps the job). Only strings cross threads (`Vm` stays `!Send`); compiled chunks memoize per pool thread by source hash; worker prints are captured and replayed before the outcome; nested spawn is refused loudly. `gui_on_frame(window, fn)` fires a callback every frame (pass `null` to unset) and implies continuous repaint, so polling UIs stay responsive. VM-only: the C transpiler rejects all three with a clear error.
- **Game input and image caching (`std::gui`)** - `gui_on_key` now reports press and release (`fn(key, pressed)`; existing one-arg handlers migrate by adding the flag), new `gui_on_mouse_move(window, fn(x, y))` for cursor tracking, and version-gated texture uploads: `gui_update_image` replaces pixels and bumps a version so static images upload once ever while animated ones pay per changed frame only.
- **New widgets and window commands (`std::gui`)** - `gui_hyperlink` (opens URLs via the browser), `gui_spinner` (loading indicator), `gui_selectable` rows with `gui_set_selected`/`gui_is_selected` (clicks fire `on_click`), `gui_clipboard_copy`/`gui_clipboard_paste` (system clipboard via arboard, no window needed), `gui_on_file_drop` (dropped file paths), `gui_on_scroll` (wheel deltas), and `gui_window_fullscreen`/`maximize`/`minimize` (one-shot requests applied next frame).
- **Layout containers (`std::gui`)** - `gui_vbox`/`gui_hbox` stack children with spacing from an origin (no absolute coordinates per child), `gui_add`/`gui_detach` move widgets in and out, `gui_set_spacing`/`padding`/`align` tune the flow. Nesting works with cycle refusal; containers derive extents bottom-up from last frame's measured sizes, hide subtrees with visibility, and cascade on remove. Manual `gui_set_pos` on contained widgets loses to the next layout pass by design.
- **Grids and scroll viewports (`std::gui`)** - `gui_grid` flows children into fixed columns (rows size to tallest, columns to widest), `gui_scroll` stacks children with a virtualized viewport where only intersecting rows render each frame, plus `gui_scroll_to`/`gui_scroll_pos` for wheel-driven paging. Parents share `gui_add`/`gui_detach`/remove-cascade; scrolled-out items go invisible, never detached, so scrolling back up loses nothing.
- **Vector drawing (`std::gui`)** - `gui_canvas` regions with per-frame `gui_draw_line`/`rect`/`circle`/`text` (local coordinates, 0-255 colors). Command lists drain every render, so animated scenes redraw from `on_frame`; static scenes redraw identically. Cheaper than pixel-array sprites for shapes, HUDs, and diagrams. `gui_set_canvas_size`/`gui_get_canvas_size` track live window dimensions for auto-resizing canvases.
